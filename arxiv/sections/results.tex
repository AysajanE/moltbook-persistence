...
and effective depth-tail slope estimate \(\hat{s}_{\mathrm{depth}} \approx 0.154\), and using
the heuristic mapping \(\mu \approx \hat{s}_{\mathrm{depth}}\), gives the
heuristic expectation \(\E[N_j] \approx \mu_0/(1-\hat{s}_{\mathrm{depth}}) \approx 6.6\)
...
process is thus better characterized as ``low-incidence/fast-conditional-response'' than as
...
\textbf{Result: a low-incidence / very-fast conditional response regime.}
...
The primary survival sample includes 9,547 at-risk comments (candidate parents) (3,456 events; 6,091 censored),
...
Overall, Moltbook threads are predominantly star-shaped (mean maximum depth 1.38, median
maximum depth 1, effective depth-tail slope estimate \(\hat{s}_{\mathrm{depth}}=0.154\)), with rare bidirectional dyads (0.998\%) and
...
36.20\% on Reddit and conditional timing differs substantially (0.076 minutes vs.\ 39.3 minutes median).
...
binning choices.
Taken together, these results jointly evaluate the model-prediction bundle
(reply-kernel timescale, depth-tail behavior, branching concentration,
reciprocity/re-entry, and periodic signatures), providing a first-pass
empirical readout of horizon-limited cascade dynamics in this early Moltbook
window.
