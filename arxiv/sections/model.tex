...
and exposure/resurfacing through \(\alpha_i\). These mechanisms are linked to
direct-reply incidence, conditional reply speed, effective depth-tail slope
estimate \(\hat{s}_{\mathrm{depth}}\), and spectral power near \(1/\tau\).
Reply-kernel half-life \((\ln 2/\beta)\) is retained as a secondary timescale
diagnostic. The depth-tail metric is descriptive (bound-motivated) and not an
exact branching-ratio identification; branching interpretations remain
heuristic.
\end{remark}
