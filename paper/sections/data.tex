...
Author identifiers are nearly complete but not perfect. In the run-scoped
thread-level artifact used for manuscript tables
(\texttt{thread\_metrics.parquet}; run \texttt{20260206-145240Z}),
906 of 223,317 comments (0.41\%) have missing \texttt{agent\_id}, concentrated in
254 threads where all comment authors are missing (0.73\% of threads). We retain
these rows in canonical thread reconstruction, count resolved commenter IDs only
for participant metrics, and treat author-based interaction metrics as undefined
when no commenter IDs are observed.
