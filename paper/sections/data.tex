...
comment identifier (\texttt{id}), a foreign key to the parent post
(\texttt{post\_id}), an author identifier (\texttt{agent\_id}), a parent-comment
identifier (\texttt{parent\_id}, null for direct replies to the post), an ISO 8601
timestamp (\texttt{created\_at}), and an observed score snapshot (\texttt{score}).
...
To contextualize Moltbook dynamics, we analyze a curated Reddit corpus collected
via the Arctic Shift archive (run identifiers in \Cref{sec:reproducibility}).

\subsubsection{Subreddit Selection}

The Reddit corpus includes six subreddits:
\texttt{r/MachineLearning}, \texttt{r/Python}, \texttt{r/artificial},
\texttt{r/datascience}, \texttt{r/learnprogramming}, and \texttt{r/programming}.

\subsubsection{Matched Sample Construction}

Matched cross-platform construction follows \Cref{sec:methods:comparison}
(implementation details in \Cref{sec:reproducibility}). Using deterministic topic mapping, UTC posting-hour bins, and
first-30-minute engagement bins, the run yields 813 one-to-one matched pairs across 118 shared
strata.

\subsubsection{Data Collection}

The corpus contains 1,772 curated submissions and 9,878
curated comments, with 1,104 threads containing at least one comment. Comment timestamps span
2026-01-31T00:03:20Z to 2026-02-04T23:59:34Z (UTC). Full run metadata are recorded in the
run manifest (see \Cref{sec:reproducibility}).

\subsubsection{Validation and Curation Caveats}

Run-scoped validation passed (no failed checks).
...
We also report a coarse matched observational comparison in
\Cref{sec:results:comparison}.
