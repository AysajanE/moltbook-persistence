...
\begin{itemize}
    \item Mean branching factor by depth level, $\bar{b}_k := \E[\text{children of nodes at depth } k]$,
    \item Root branching factor (direct replies to the post),
    \item Distribution of branching factors across all nodes.
\end{itemize}

Star-shaped trees (many direct replies, few deep chains) exhibit high root branching factor and rapidly declining $\bar{b}_k$.
...
\begin{equation}
\label{eq:hazard}
h(s \mid t_{jm}) = b(t_{jm} + s) \, \alpha_{a_{jm}} \, e^{-\beta_{a_{jm}} s}.
\end{equation}
...
\end{equation}
where $x_i$ includes covariates (log-karma, log-follower count, account age). We estimate via penalized likelihood or Bayesian MCMC.
...
As described in \cref{sec:data:reddit}, we employ coarsened exact matching on early engagement, topic, and posting time. This yields a matched sample where Moltbook and Reddit posts have similar initial conditions, isolating the effect of platform (agent vs.\ human).
...
All analyses are implemented in Python using:
\begin{itemize}
    \item \texttt{pandas} and \texttt{polars} for data manipulation,
    \item \texttt{lifelines} for survival analysis,
    \item \texttt{scipy.signal} for spectral analysis,
    \item \texttt{statsmodels} for regression and time-series models,
    \item \texttt{networkx} for tree operations.
\end{itemize}

Code and intermediate outputs are versioned in a public repository (see \cref{sec:reproducibility}).
