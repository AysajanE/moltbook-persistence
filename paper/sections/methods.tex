...
\(N_{\mathrm{risk}}\) is the number of at-risk parent comments and
...
parent comment to end of observable coverage), and
...
\item \textbf{Parent comment (candidate parent / at-risk comment):} any non-root comment that can receive a direct reply; each such candidate contributes one survival unit.
\item \textbf{Missing author-ID handling:} \texttt{n\_unique\_agents} counts non-null
commenter IDs only; re-entry uses known-comment denominators
(\texttt{known\_agent\_comment\_count}); reciprocity uses only edges where both
source and parent IDs are observed; when no commenter IDs are known in a thread,
author-based thread metrics are left undefined (NA) rather than imputed.
\end{itemize}
\end{minipage}%
}
\end{center}
...
reply-kernel half-life \(h=\ln 2 / \beta\). For each at-risk parent comment
