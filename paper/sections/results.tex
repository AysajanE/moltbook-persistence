...
Moltbook conversation trees are shallow (\cref{fig:depth-distribution}): mean maximum depth is 1.378
(95\% bootstrap CI: [1.372, 1.383]), median maximum depth is 1, the proportion reaching depth 5+
is 0.0058\% (95\% bootstrap CI: [0.0000\%, 0.0144\%]), and the proportion reaching depth 10+
is 0.0000\%.
...
9.00\% of at-risk parent comments receive any direct reply in-window, versus 36.20\% on the
...
Mean maximum depth per thread is 2.169 (median maximum depth 1),
\(\Prob(D \ge 5)=11.32\%\),
\(\Prob(D \ge 10)=1.54\%\), mean branching factor is 4.305 at depth 0 and 0.446 at depth 1,
and re-entry has mean 0.0938 and median 0.
...
gives a reply-kernel half-life of 2.6066 hours (95\% CI: [2.2933, 2.9539]). A no-boundary
...
This section is a secondary, context-setting analysis rather than a core
causal identification pillar of the paper. We execute coarse exact matching on
the Moltbook and Reddit datasets described in \Cref{sec:methods:comparison} to
compare outcomes only within shared support.
...
(813 threads per platform; \cref{fig:cross-platform-flow}). Only 2.34\% of Moltbook threads
are in the final matched sample (813/34,730), so matched estimates should be interpreted as
describing the overlap region rather than the full platform population or a
platform-wide causal effect.
...
All two-sided Wilcoxon signed-rank tests reject at conventional levels
(\(p < 10^{-8}\) for each outcome).
This includes re-entry, where the matched contrast is negative even though the
overall full-sample descriptive contrast is positive (Moltbook 0.195 vs.\ Reddit
0.0938), indicating that conditioning on overlap strata can change direction.
...
\textbf{Matched-subset reply-kernel half-life.} Restricting survival units to matched threads,
the primary-sample half-life is 0.0628 hours on Moltbook (3.77 minutes; 95\% CI:
[1.19, 7.06] minutes; \(n=1{,}841\) at-risk parent comments, 22 events) and 2.4448 hours on Reddit
...
thread-level survival effects, and should be read as contextual overlap-region
contrasts.
Because the Moltbook matched subset has only 22 events and narrow overlap
support, its half-life estimate is noisy and not directly comparable with the
overall Moltbook estimate.

\subsection{Summary of Key Findings}
\label{sec:results:summary}

Overall, Moltbook threads are strongly star-shaped (mean maximum depth 1.378, median
...
specification yields a very short reply-kernel half-life of 0.0133 hours
...
In the coarse matched cross-platform design, exact matching on topic, UTC
posting hour, and first-30-minute engagement yields 813 pairs across 118 shared
strata, and the maximum absolute SMD decreases from 3.886 to 0.052. Within this
matched sample, Moltbook threads are lower on comments (\(-7.704\)), depth
(\(-1.191\)), unique participants (\(-5.009\)), duration (\(-8.606\) hours),
and re-entry (\(-0.040\)); all corresponding Wilcoxon tests are statistically
significant. This re-entry direction differs from the overall full-sample
descriptive contrast, showing that overlap conditioning can reverse the sign.
These matched differences are secondary contextual estimates on overlap support,
not platform-wide causal effects. On matched-thread subsets,
Moltbook reply-kernel half-life is
0.0628 hours (3.77 minutes; 95\% CI: [1.19, 7.06] minutes), versus 2.4448 hours
for Reddit (95\% CI: [2.1298, 2.7926]); with only 22 Moltbook events in this
matched subset, that half-life is noisy and not directly comparable to the
overall estimate. For Reddit periodicity, the dominant
component is 32 hours (0.03125 hr\(^{-1}\)); the 4-hour target-frequency test
is not significant (\(p=0.6845\)), while Fisher's \(g\) reports \(p<0.0005\)
(0/2000 exceedances).
