...
\(\pm 0.03\) hr\(^{-1}\) neighborhood around \(f_\tau^\star\). However, first-order autoregressive (AR(1))-calibrated tests do not support significance
...
size (power \(=0.0496\) at \(\kappa=0\)). The estimated 80\%-power
detectability threshold is \(\kappa^\star=0.2\), so this window would detect a
4-hour phase concentration at or above \(\kappa=0.2\) with at least 80\%
probability under the simulation design.
...
summary, the paired-effects table, and matched-subset half-life details are
reported in \Cref{sec:appendix:comparison-details,sec:appendix:match-halflife}.
