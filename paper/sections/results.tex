...
We present five results: (1) thread geometry, (2) persistence decomposition
into incidence versus conditional timing, (3) model-to-data consistency checks,
(4) periodicity tests as secondary evidence, and (5) Reddit baseline with
overlap-restricted matching as contextual comparison.
...
(2026-02-02 04:20:50Z to 2026-02-04 19:51:53Z; 63.5 hours).

The primary test is event-time modulo-4-hour Rayleigh concentration. On this
segment (\(N=220{,}461\) events), resultant length is \(r=0.0308\), with
\(Z=209.57\), Monte Carlo \(p=5\times 10^{-6}\), and circular mean phase
153.2 minutes. Although statistically non-uniform, the concentration magnitude
is small (\(\approx 3.1\%\) of perfect phase alignment).

Detectability simulations (same \(N\), null calibration, and decision rule)
show near-nominal null size (power \(=0.0496\) at \(\kappa=0\)) and an
80\%-power threshold of \(\kappa^\star=0.2\). Therefore, this window has high
power for moderate/strong 4-hour concentration but limited power for weak
concentration below \(\kappa=0.2\). The observed \(r=0.0308\) is far below this
detectability scale.
...
9.60\% direct-reply incidence (95\% CI: [9.45\%, 9.76\%]) with very fast
...
Model-to-observable validation shows tight
