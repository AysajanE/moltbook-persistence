...
two-part sample includes 223,316 parents. Primary incidence is
horizon-standardized: \(p_{5\mathrm{m}}=9.42\%\) and
\(p_{1\mathrm{h}}=9.82\%\). The in-window ever-reply share is 9.60\%
(95\% bootstrap CI: [9.45\%, 9.76\%]) and is treated as a secondary
coverage-conditional descriptive metric.
...
\(p=5\times10^{-6}\)). Because \(r\) is far below the detectability benchmark
\(\kappa^\star=0.2\), we interpret this as weak aggregate coherence rather than
a practically strong four-hour synchronization signal. Supplementary PSD/AR(1)
and bin-width diagnostics are consistent with this interpretation.
...
estimators. These baseline contrasts are descriptive and non-causal and are
used only as contextual evidence for H2 interpretation; detailed mechanics and
diagnostics are deferred to supplementary material.
