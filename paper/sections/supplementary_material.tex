...
The null calibration uses 200,000 Monte Carlo draws at \(\alpha=0.05\), giving
critical \(Z=2.9940\).
...
Estimated 80\% power threshold \(\kappa^\star\) & 0.2 \\
...
\(\kappa=0.2\). This section is the detailed detectability record deferred from
main text.
...
diagnostic values (2.61 hours; 95\% CI: [2.29, 2.95]) than Moltbook.
...
\texttt{paper/tables/cross\_platform\_matched\_paired\_effects.csv}.}
\label{tab:cross-platform-paired-effects}
\scriptsize
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Outcome} & \textbf{\(n\) pairs} & \textbf{Mean diff. (M-R)} & \textbf{95\% CI} & \textbf{Wilcoxon \(p\)} \\
\midrule
...
\paragraph{S6.1 Timing fit diagnostic table (moved from main).}
\begin{table}[h]
\centering
\caption{Observed vs.\ fitted timing-model diagnostics for conditional reply
times and event probability (migrated from main results).}
