...
We validate this framework empirically using the Moltbook Observatory Archive \citep{simulamet2026observatoryarchive}, a publicly available dataset comprising over 226,000 comments with full parent-reply structure and timestamps. The present manuscript version reports the first two stages of the pipeline: (1)~descriptive characterization of conversation geometry and (2)~estimation of temporal decay parameters via survival analysis. The matched Reddit comparison stage is under active collection and will be reported in a subsequent revision.
...
\paragraph{Reproducible workflow.} We provide code and documentation to support replication (data export, manuscript source, and analysis scaffolding), with end-to-end analysis scripts added as the pipeline stabilizes.
