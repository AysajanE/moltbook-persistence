...
\begin{enumerate}
    ...
    \item \textbf{Conversation geometry:} What structural properties (depth, branching, reciprocity) characterize Moltbook discussion trees, and how do these compare to human-platform baselines?
...
\begin{enumerate}
    \item \textbf{H1 (Short half-life and attention clock).} We test whether Moltbook threads exhibit characteristically short first-reply half-lives consistent with architectural constraints, and whether aggregate comment activity shows periodic structure near the hypothesized heartbeat cadence ($\tau \approx 4$ hours).
    \item \textbf{H2 (Shallow, star-shaped structure).} We test whether Moltbook comment trees are shallower and more root-concentrated than human-platform baselines, with lower reciprocity and re-entry rates.
    \item \textbf{H3 (Topic moderates persistence).} We test whether persistence varies by topic/community, with builder/technical submolts exhibiting longer half-lives and deeper trees than social or low-signal submolts.
    \item \textbf{H4 (Agent heterogeneity).} We test whether agent-level covariates (account claim status, follower count) predict variation in reply incidence and conversational persistence.
\end{enumerate}
...
We evaluate this framework empirically using the Moltbook Observatory Archive \citep{simulamet2026observatoryarchive} and a run-scoped curated Reddit corpus as a contextual human-platform baseline. The present manuscript reports: (1)~descriptive characterization of conversation geometry, (2)~estimation of temporal decay parameters via survival analysis, (3)~spectral tests for periodic activity signatures, (4)~a full-scale Reddit-side baseline analysis, and (5)~a coarse matched observational comparison with paired effect estimation.
