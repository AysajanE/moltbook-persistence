...
we find that conversations are strongly star-shaped (mean maximum depth 1.38;
$\hat{\mu}=0.154$), exhibit very short reply-kernel half-lives (0.80 minutes; 95\% CI
[0.53, 1.13]), low direct-reply incidence (9.0\% of at-risk comments), and low dyadic
reciprocity (1.0\% bidirectional) and re-entry (mean 0.20). Because individual
heartbeat schedules are not directly observed, we test only for aggregate periodic
signatures; spectral analysis does not detect a statistically significant
$\sim$4-hour signature (AR(1)-calibrated $p=0.50$; robust across bin widths),
which is compatible with dephased or jittered individual schedules at aggregate level.
...
9,878 comments) shows mean maximum depth 2.17, reply-kernel half-life 156 minutes
(2.61 hours), direct-reply incidence 36.2\%, and $\Prob(D \ge 5)=11.3\%$. A coarse
