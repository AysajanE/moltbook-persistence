...
The availability function captures platform-level scheduling. On Moltbook, the heartbeat mechanism instructs agents to check the platform at approximately 4-hour intervals, inducing periodic structure in $b(t)$. A tractable parametric form is:
\begin{equation}
...
The decay rate $\beta_i$ captures how quickly attention to agent $i$'s content fades. Faster context loss or higher task-switching costs correspond to larger $\beta_i$. The combined effect of availability and staleness enters multiplicatively in the event intensities we now define.
...
\label{rem:hawkes}
If we ignore explicit parent-edge generation and consider only event times, \cref{eq:total-intensity} defines a marked Hawkes process with agent-dependent excitation kernel $g_i(\Delta) = \alpha_i e^{-\beta_i \Delta}$ modulated by $b(t)$. Such processes are widely used for information cascades and social media dynamics \citep{crane2008robust, zhao2015seismic, rizoiu2017expecting}.
\end{remark}
...
Under the direct-reply model \cref{eq:reply-intensity}, inter-reply times to a fixed parent $m$ form an inhomogeneous Poisson process with hazard proportional to $e^{-\beta_{a_{jm}} \Delta}$. The decay rate $\beta_i$ can thus be estimated via maximum likelihood using offspring times relative to each parent, treating $b(t)$ as a known offset. \Cref{sec:methods} details our estimation procedure.
\end{remark}
...
\begin{proposition}[Expected thread size]
\label{prop:thread-size}
Let $Z_k$ denote the number of nodes at depth $k$. Under the single-type approximation with mean offspring $\mu < 1$:
\begin{equation}
\label{eq:expected-size}
\E[|T_j|] = \sum_{k=0}^\infty \E[Z_k] = \sum_{k=0}^\infty \mu^k = \frac{1}{1 - \mu}.
\end{equation}
The expected number of comments (excluding the root) is $\E[N_j] = \mu / (1 - \mu)$.
\end{proposition}
...
\end{equation}
where $u_i^{(\cdot)}$ are mean-zero latent heterogeneity terms (e.g., Gaussian). This hierarchical specification is standard in survival and point-process modeling \citep{cox1972regression}.
...
The re-entry half-life for agent $i$ is $h_i^{(r)} = \ln 2 / \beta_i^{(r)}$. Agents with large $\eta_i$ and small $\beta_i^{(r)}$ are ``coordination hubs'' that sustain extended threads.
...
These predictions are testable via spectral analysis of timestamps and autocorrelation of per-agent activity sequences. Detection of a $\sim$4-hour periodic component would provide direct evidence linking the heartbeat mechanism to observed dynamics.
...
If clients display recent items prominently, the parent-selection distribution \cref{eq:parent-selection} can be extended with position-dependent weights $w_{jm}(t)$. Higher recency weights predict more star-shaped trees (many replies to near-root nodes). Estimating $w$ from reply patterns quantifies how interface design shapes structure.
...
\midrule
Short interaction half-life ($\sim$4 hours) & Heartbeat cadence + context limits & Survival analysis of inter-reply times \\
Shallow conversation trees & High $\beta$ $\Rightarrow$ low $\mu$ $\Rightarrow$ exponential depth decay & Depth distribution analysis \\
Star-shaped structure & Recency bias in parent selection & Branching factor by depth level \\
Periodic activity patterns & Heartbeat synchronization & Spectral analysis of timestamps \\
Heterogeneous re-entry & Few high-$\eta$ agents sustain threads & Re-entry rate distribution \\
