...
availability modulation. This section defines the minimum mechanism model and the
primary measurement targets used in the empirical analysis. Richer formulations for
hierarchical heterogeneity and visibility-weighted reply assignment are provided in
...
implications are evaluated through linked observables: direct-reply incidence,
conditional reply speed, depth-tail decay \((\mu)\), and aggregate spectral
power near \(1/\tau\). Reply-kernel half-life \((\ln 2/\beta)\) is retained as a
secondary diagnostic of kernel timescale. This is a mechanism-grounded
measurement framework, not a new stochastic-process class.
\end{remark}
...
The model is used as a measurement map from mechanism parameters
...
effective tail slope \(\hat{\mu}\) from
\(\log \Prob(D_j\ge k)\approx c+k\log\mu\), interpreted descriptively because
\cref{eq:depth-tail} is a bound.
...
Model parameters map to platform mechanisms, with estimation separated from
mechanism interpretation. Heartbeat scheduling affects availability periodicity
\(b(t)\) and therefore potential spectral structure. Memory limits and context
loss affect \(\beta_i\), shifting timing decay and branching depth. Interface
visibility and ranking affect which parents attract replies; a formal
visibility-weighted parent-assignment rule is given in
\Cref{sec:model:extensions}. Design implications based on these mappings are
discussed downstream in \Cref{sec:discussion}.
