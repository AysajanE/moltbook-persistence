...
This formulation combines standard Hawkes-style self-excitation and branching-process
components with exponential reply-kernel decay. The novel contribution in this manuscript
is the measurement map from platform mechanisms to persistence estimands in observatory
data: exposure/resurfacing enters through \(\alpha\) (\cref{eq:alpha-exposure}),
context-window staleness through \(\beta\), and check-in cadence/synchronization through
\(b(t)\). Empirically, we estimate direct-reply incidence and conditional timing, and
report reply-kernel half-life as a secondary timescale diagnostic. By contrast, depth-tail
slope summaries, two-type branching interpretations, and
\(p_\infty=1-\exp(-\hat\alpha/\hat\beta)\) are descriptive or model-implied diagnostics
rather than identified causal estimands. Periodicity is tested primarily with
event-time modulo-\(\tau\) concentration and explicit detectability thresholds;
PSD/autocorrelation checks are supplementary diagnostics. Platform-lever comparative
statics are interpreted as mechanism-consistent hypotheses. Because per-account
heartbeat events are not directly logged, \(b(t)\) is latent and inferred from
aggregate periodic signatures.
...
\(g_i(\Delta)=\alpha_i e^{-\beta_i\Delta}\) modulated by \(b(t)\); this construction
follows standard self-exciting process practice in social-media dynamics
\citep{crane2008robust,zhao2015seismic,rizoiu2017expecting}.
...
component).
This decomposition separates the ``whether'' margin from the ``when''
margin, so primary persistence conclusions do not require the exponential
kernel in \cref{eq:reply-intensity} to be an exact timing law.
...
minimum phase-concentration amplitude that is detectable with prespecified
power in modulo-\(\tau\) testing.
\end{itemize}
We also report
...
methods from parent-relative reply times; each non-root candidate parent comment
is an at-risk unit (``at-risk comment'').
...
Because \cref{eq:depth-tail} is an upper bound, the effective depth-tail slope
estimate \(\hat{s}_{\mathrm{depth}}\), obtained from
\(\log \Prob(D_j\ge k)\approx c+k\log s_{\mathrm{depth}}\), is interpreted as a
descriptive summary motivated by the bound, not as an exact
...
\begin{proposition}[Periodic mean intensity and detectability]
...
In main text, we operationalize this with one primary inferential test:
event-time modulo-\(\tau\) phase concentration (Rayleigh). PSD and
autocorrelation checks are treated as supplementary diagnostics
(\Cref{sec:appendix:periodicity-robustness}).
