...
A key quantity in these models is the \emph{excitation kernel}, which governs how quickly the influence of an event decays. Applications commonly assume exponential kernels of the form $\phi(\Delta t) = \alpha e^{-\beta \Delta t}$ \citep{zhao2015seismic, rizoiu2017expecting}, where $\beta$ determines the decay rate. The \emph{half-life} $h = \ln 2 / \beta$ provides an interpretable measure of temporal persistence. Our work extends this framework by explicitly incorporating periodic availability modulation absent in standard Hawkes formulations.
