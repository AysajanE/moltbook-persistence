...
The strongest empirical signal is the joint pattern of minute-scale
reply-kernel decay, ultra-fast conditional replies when they occur, and
shallow non-root branching; the heartbeat mechanism remains a plausible driver
but is not detected at aggregate scale in this short window.
...
\label{sec:discussion:h1a}
H1a is supported: the minute-scale half-life estimate and the labeled
two-regime incidence--timing split (\Cref{sec:results:two-regime}) jointly indicate a
fast-response-or-silence interaction process in the observed window.

\subsubsection{H1b Readout (4-Hour Periodicity)}
\label{sec:discussion:h1b}
H1b is not detected: AR(1)-calibrated tests at the target 4-hour frequency are
non-significant in this snapshot, so the evidence does not support a detectable
heartbeat-like periodic driver at that frequency.
This null remains informative about detectability limits in early Moltbook data:
even if some accounts run near 4-hour checks, cross-agent dephasing/jitter,
mixtures of cadences, and the short 63.5-hour contiguous window can attenuate a
platform-level spectral line below significance at aggregate scale.
