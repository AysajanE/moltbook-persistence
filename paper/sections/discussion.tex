...
half-life (2.6066 hours in the primary specification). Because matched cross-platform
inference is not yet executed, these platform-side differences should be treated as
descriptive context rather than causal platform effects.
...
\paragraph{Human-AI comparison.} The manuscript now includes a run-scoped Reddit baseline and
Moltbook estimates computed with aligned metrics. The next stage is matched inference
(coarsened exact matching and paired effect estimation) to quantify cross-platform differences
with stronger control for selection effects.
